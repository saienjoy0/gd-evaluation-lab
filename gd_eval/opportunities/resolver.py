...
def _after_initial_positions(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    prior_ai = {
        message.get("participant_id")
        for message in _before(_messages(episode), event["timestamp_ms"])
        if message.get("speaker_type") == "ai"
    }
    expected = {participant["agent_id"] for participant in scenario.get("ai_participants", [])}
    return bool(expected) and expected <= prior_ai


def _after_goal_question(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _before_idea_generation(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    starts = [
        message["start_ms"]
...
def _after_two_options_present(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    ...
    return any(message.get("move") == "compare_options" for message in prior) or len(
        {message.get("participant_id") for message in prior if message.get("speaker_type") == "ai"}
    ) >= min(2, len(scenario.get("ai_participants", [])))


def _after_constraint_reveal(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _after_ai_question(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
        message.get("speaker_type") == "ai" and message.get("move") == "ask_question"
        for message in _before(_messages(episode), event["timestamp_ms"])
    )


def _after_private_concern_reveal(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    return _after_constraint_reveal(scenario, episode, event)


def _after_initial_ideas(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _after_tradeoff_identified(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _after_position_conflict(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    prior = _before(_messages(episode), event["timestamp_ms"])
...
def _before_final_selection(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _before_session_close(scenario: dict[str, Any], episode: dict[str, Any], event: dict[str, Any]) -> bool:
    del scenario
    return any(
...
def _context_satisfied(
    ...
    checks: dict[str, bool] = {
        ...
        "two_options_available": any(message.get("move") == "compare_options" for message in prior)
        or len({message.get("participant_id") for message in prior if message.get("speaker_type") == "ai"}) >= 2,
        ...
        "idea_space_open": any(message.get("phase") == "idea_generation" for message in prior + future),
        ...
        "criteria_and_options_available": any(
            message.get("speaker_type") == "user" and message.get("move") == "define_criteria"
            for message in prior
        )
        and any(message.get("move") == "compare_options" for message in prior),
        "remaining_time_visible": timestamp < max(
            (message.get("end_ms", 0) for message in _messages(episode)), default=timestamp
        ),
    }
    if context not in checks:
        raise OpportunityResolutionError(f"UNIMPLEMENTED_OPPORTUNITY_CONTEXT: {context}")
...
def resolve_opportunities(
    ...
    for opportunity in scenario.get("evaluation_opportunities", []):
        ...
        events = sorted(offers.get(opportunity_id, []), key=lambda item: item["timestamp_ms"])
        invalidated_by = [
            condition
        ...
        for event in events:
            ...
            if handler is None:
                raise OpportunityResolutionError(f"UNIMPLEMENTED_OPPORTUNITY_TRIGGER: {trigger}")
            if not handler(scenario, episode, event):
                raise OpportunityResolutionError(f"OPPORTUNITY_TRIGGER_INVALID: {opportunity_id}")
            if not all(
                _context_satisfied(
                ...
                raise OpportunityResolutionError(f"OPPORTUNITY_CONTEXT_INVALID: {opportunity_id}")
        ...
        for event in events:
            for message_id in event.get("candidate_response_message_ids", []):
                ...
                if message.get("start_ms", 0) < event.get("timestamp_ms", 0):
                    raise OpportunityResolutionError(
                        f"OPPORTUNITY_RESPONSE_BEFORE_TRIGGER: {opportunity_id}:{message_id}"
                    )
